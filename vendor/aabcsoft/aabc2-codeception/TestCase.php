...
use aabc\base\UnknownMethodException;
use aabc\base\UnknownPropertyException;
use aabc\di\Container;
use aabc\test\ActiveFixture;
use aabc\test\BaseActiveFixture;
...
class TestCase extends Test
{
     ...
     * name of the application instance to be created. By default, a [[\aabc\web\Application]] instance will be created.
     */
     ...
     * @return \aabc\web\Application|\aabc\console\Application the application instance
    ...
    protected function mockApplication($config = null)
    {
        ...
        Aabc::$container = new Container();
        $config = $config === null ? $this->appConfig : $config;
        if (is_string($config)) {
            $configFile = Aabc::getAlias($config);
    ...
    {
        if (\Aabc::$app) {
            if (\Aabc::$app->has('session', true)) {
                \Aabc::$app->session->close();
            }
            if (\Aabc::$app->has('db', true)) {
                Aabc::$app->db->close();
            }
        }
        Aabc::$app = null;
