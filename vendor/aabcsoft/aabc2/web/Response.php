...
class Response extends \aabc\base\Response
{
    ...
    protected function sendCookies()
    {
        ...
        foreach ($this->getCookies() as $cookie) {
            ...
            if ($cookie->expire != 1  && isset($validationKey)) {
                $value = Aabc::$app->getSecurity()->hashData(serialize([$cookie->name, $value]), $validationKey);
            }
    ...
    public function redirect($url, $statusCode = 302, $checkAjax = true)
    {
        ...
        if (strpos($url, '/') === 0 && strpos($url, '//') !== 0) {
            $url = Aabc::$app->getRequest()->getHostInfo() . $url;
        }

        if ($checkAjax) {
            if (Aabc::$app->getRequest()->getIsAjax()) {
                if (Aabc::$app->getRequest()->getHeaders()->get('X-Ie-Redirect-Compatibility') !== null && $statusCode === 302) {
                    // Ajax 302 redirect in IE does not work. Change status code to 200. See https://github.com/aabcsoft/aabc2/issues/9670
                    $statusCode = 200;
                }
                if (Aabc::$app->getRequest()->getIsPjax()) {
    ...
    {
        return $this->redirect(Aabc::$app->getRequest()->getUrl() . $anchor);
    }
    ...
    {
        return [
            self::FORMAT_HTML => 'aabc\web\HtmlResponseFormatter',
            self::FORMAT_XML => 'aabc\web\XmlResponseFormatter',
            self::FORMAT_JSON => 'aabc\web\JsonResponseFormatter',
            self::FORMAT_JSONP => [
                'class' => 'aabc\web\JsonResponseFormatter',
                'useJsonp' => true,
            ],
        ];
    }
    ...
    protected function prepare()
    {
        ...
        if (isset($this->formatters[$this->format])) {
            ...
            if (!is_object($formatter)) {
                $this->formatters[$this->format] = $formatter = Aabc::createObject($formatter);
            }
