...
class HelpController extends Controller
{
    ...
    {
        if ($command !== null) {
            $result = Aabc::$app->createController($command);
            if ($result === false) {
    ...
    {
        $result = Aabc::$app->createController($action);
    ...
    {
        $commands = $this->getModuleCommands(Aabc::$app);
        sort($commands);
    ...
    protected function validateControllerClass($controllerClass)
    {
        if (class_exists($controllerClass)) {
            ...
            return !$class->isAbstract() && $class->isSubclassOf('aabc\console\Controller');
        } else {
            return false;
        }
    }
    ...
    protected function getSubCommandHelp($controller, $actionID)
    {
        ...
        $options = $controller->getActionOptionsHelp($action);
        $options[\aabc\console\Application::OPTION_APPCONFIG] = [
            'type' => 'string',
            'default' => null,
    ...
    {
        return basename(Aabc::$app->request->scriptFile);
    }

    
    protected function getDefaultHelpHeader()
    {
        return "\nThis is Aabc version " . \Aabc::getVersion() . ".\n";
    }
}
